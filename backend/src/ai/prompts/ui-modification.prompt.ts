...
- When the user asks to change, swap, or randomize the background (the effect/component) without naming a specific ReactBits component, replace the background with a different component from the full catalog — do not default to SoftAurora. Never use PrismaticBurst or Ribbons (removed from library).
...
- Targeted edits: when the user asks to change a specific element's color, text, size, or style, change ONLY that element (and its direct className/style). Locate elements by matching the exact text content the user names (e.g. heading "AI Technology Meetup").
- For color requests, apply Tailwind classes (e.g. text-green-500, text-emerald-600) or style={{ color: '...' }} on the matching element — do not leave the old color class on the same element.
- Do not revert or override explicit color or style changes requested by the user.
